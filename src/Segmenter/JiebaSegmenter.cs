...
namespace JiebaNet.Segmenter
{
    public class JiebaSegmenter
    {
        ...
        private static readonly SemaphoreSlim asyncLocker = new SemaphoreSlim(1, 1);
        ...
        /// 加载用户词典（同步版本）
        /// </summary>
        /// <param name="userDictFile">用户词典文件路径</param>
        ...
        /// </summary>
        /// <param name="userDictFile">用户词典文件路径</param>
        public async Task LoadUserDictAsync(string userDictFile)
        {
            var dictFullPath = Path.GetFullPath(userDictFile);
            Debug.WriteLine("Initializing user dictionary (async): " + userDictFile);

            await asyncLocker.WaitAsync().ConfigureAwait(false);
            try
            {
                if (LoadedPath.Contains(dictFullPath))
                    return;
                ...
                try
                {
                    ...
                    var lines = await File.ReadAllLinesAsync(dictFullPath, Encoding.UTF8).ConfigureAwait(false);
                    foreach (var line in lines)
                    {
                        if (string.IsNullOrWhiteSpace(line))
                        {
                    ...
                    Debug.WriteLine("user dict '{0}' async load finished, time elapsed {1} ms",
                        dictFullPath, DateTime.Now.Millisecond - startTime);
                }
                catch (IOException e)
                {
