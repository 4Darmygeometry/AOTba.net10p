// 所有框架都使用正则表达式识别日期时间
// 混合策略：编译期确定的正则使用源生成器[GeneratedRegex]，其余使用预编译正则
...
namespace JiebaNet.Segmenter
{
    /// <summary>
    /// 基于正则表达式的时间识别器
    /// 混合策略：编译期确定的正则使用源生成器，引用C_常量/GB18030_2022的正则使用预编译
    /// </summary>
    public partial class RegexTimeRecognizer : ITimeRecognizer
    {
        ...
        public static partial Regex DateTimeIsoRegex { get; }

        // ========== 2. 中文日期时间（统一匹配年份、年月、年月日、月日及日期时间组合） [预编译正则] ==========
        // 匹配格式：
        ...
        // ========== 6. 时间（严格格式：HH:MM:SS.ffff、HH:MM:SS、HH:MM，时分秒60进制） [预编译正则] ==========
        ...
        [GeneratedRegex(
            ...
            @"(?![\d.])")]
        public static partial Regex RatioRegex { get; }
        ...
        // 编译期完全确定，无C_常量或GB18030_2022引用
        [GeneratedRegex(@"\d+(?:\.\d+)?%")]
        public static partial Regex PercentageRegex { get; }
        ...
        public static partial Regex HyphenatedWordRegex { get; }
